    ...
    try {
        ...
        for (let i = 0; i <= width; i += 2) {
            ...
            if (cropX >= width || cropWidth <= 0) {
                console.error(`Invalid crop area: cropX=${cropX}, cropWidth=${cropWidth}`);
                continue;
            }
...
async function getDragonStrip(dragonIds) {
    const dragonImages = await Promise.all(dragonIds.map(async (dragonId) => {
        const response = await fetch(`https://dragcave.net/image/${dragonId}.gif`);
        const arrayBuffer = await response.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        return sharp(buffer, { animated: true });
    }));
    const dragonMetadatas = await Promise.all(dragonImages.map((dragonImage) => dragonImage.metadata()));
    const totalWidth = dragonMetadatas.reduce((acc, curr) => acc + (curr.width ?? 0) + 1, 0);
    const maxHeight = Math.max(...dragonMetadatas.map((dragon) => dragon.height ?? 0));
