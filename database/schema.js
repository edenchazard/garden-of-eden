...
import { bigint, boolean, char, datetime, index, json, mediumint, mysqlTable, smallint, tinyint, unique, varchar, text, } from 'drizzle-orm/mysql-core';
...
export const hatcheryTable = mysqlTable('hatchery', {
    ...
    return {
        in_seed_trayIdx: index('in_seed_tray_idx').on(table.in_seed_tray, table.user_id),
...
export const clicksTable = mysqlTable('clicks', {
    ...
    return {
        hatchery_id_user_idIdx: unique('hatchery_id_user_id_idx').on(table.hatchery_id, table.user_id),
        user_id_clicked_onIdx: index('user_id_clicked_on_idx').on(table.user_id, table.clicked_on),
        clicked_onIdx: index('clicked_on_idx').on(table.clicked_on),
    };
});
export const clicksLeaderboardTable = mysqlTable('clicks_leaderboard', {
    user_id: mediumint('user_id', {
        unsigned: true,
    })
        .references(() => userTable.id, {
    ...
    return {
        ...
        leaderboard_start_user_idIdx: unique('leaderboard_start_user_idIdx').on(table.leaderboard, table.start, table.user_id),
    };
});
...
export const bannerJobsTable = mysqlTable('banner_jobs', {
    ...
    dragonsIncluded: json('dragons_included').$type(),
    dragonsOmitted: json('dragons_omitted').$type(),
...
export const userSettingsSchema = createSelectSchema(userSettingsTable, {
    gardenFrequency: (schema) => schema.gardenFrequency.min(15).max(300).default(30),
    gardenPerPage: (schema) => schema.gardenPerPage.min(10).max(500).default(500),
    seedTrayFrequency: (schema) => schema.seedTrayFrequency.min(15).max(300).default(30),
    seedTrayPerPage: (schema) => schema.seedTrayPerPage.min(10).max(500).default(200),
    ...
    highlightClickedDragons: (schema) => schema.highlightClickedDragons.default(true),
