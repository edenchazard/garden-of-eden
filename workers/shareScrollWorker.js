...
parentPort?.on('message', async function (message) {
    if (message.type !== 'banner')
        return;
    const { user, filePath, weeklyClicks, weeklyRank, allTimeClicks, allTimeRank, dragonCodes, } = message;
    try {
        await generateBannerToTemporary(user, filePath, weeklyClicks, weeklyRank, allTimeClicks, allTimeRank, dragonCodes);
...
// the meat of it
async function generateBannerToTemporary(user, filePath, weeklyClicks, weeklyRank, allTimeClicks, allTimeRank, dragonCodes) {
    try {
...
async function getBannerBase(username, weeklyClicks, weeklyRank, allTimeClicks, allTimeRank, flairUrl) {
    try {
...
async function getDragonStrip(dragonIds) {
    ...
    try {
        ...
        const dragonBuffers = await Promise.all(dragonIds.map(async (dragonId) => {
            const response = await fetch(`https://dragcave.net/image/${dragonId}.gif`);
            if (!response.ok) {
                throw new Error(`Failed to fetch dragon image ${dragonId}: ${response.statusText}`);
            }
